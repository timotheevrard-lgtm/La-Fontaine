...
console.log(`📂 ${sessions.size} session(s) chargée(s) depuis le disque.`);
...
app.get("/api/sessions", (req, res) => {
  ...
  for (const [id, session] of sessions.entries()) {
    list.push({
      sessionId: id,
      title: session.title || "Histoire sans titre",
  ...
  try {
    await runAgent(messages, (event) => {
      ...
      if (event.type === "tool_result" && event.name === "create_chapter_page") {
        sessions.get(sessionId).chapterCount++;
        saveSessions(sessions);
      }
    });
    sessions.get(sessionId).messages = messages;
    saveSessions(sessions);
...
app.post("/api/chapter", async (req, res) => {
  ...
  const userMessage = brief
    ? `Écris le Chapitre ${chapterNum} avec ce brief : ${brief}. Développe-le en détail.`
    : `Continue l'histoire et écris le Chapitre ${chapterNum} en suivant logiquement les événements précédents. Sois très détaillé.`;
  ...
  try {
    await runAgent(session.messages, (event) => {
      send(event);
      if (event.type === "tool_result" && event.name === "create_chapter_page") {
